...
def main():
    ...
    As opposed to main_mf_sm.py which uses LHS intitial DOEs, this version uses the
    low-fidelity optimal candidates as the initial DOE:
    - LF DOE = 4 low-fidelity optimal candidates + baseline + LHS of 45 samples i.e. 50 samples
    - HF DOE = 4 low-fidelity optimal candidates (recomputed with LES) + baseline i.e. 5 samples
    ...
    # read optimal DOEs
    print("MFSM: read results..")
    n_hf = sm_config["optim"]["n_hf"]
    n_lf = sm_config["optim"]["n_lf"]
    QoI = sm_config["optim"]["QoI"]
    CoI = sm_config["optim"]["CoI"]

    # 1. the optimal DOEs are first read
    # candidates
    x_hf = np.loadtxt(sm_config["optim"]["opt_candidates"])
    x_lf = x_hf.copy()

    # HF DOE
    hf_dir = sm_config["optim"]["hf_doe_dir"]
    ...
    # LF DOE
    with open(os.path.join(hf_dir, "lf_df_dict.pkl"), "rb") as handle:
        lf_dict = pickle.load(handle)
    # QoIs
    lf_w_ADP = np.array([lf_dict[gid][cid]["ADP"][QoI].dropna().iloc[-1]
                         for gid, cid in zip(
                             sm_config["optim"]["lf_opt_gid"],
                             sm_config["optim"]["lf_opt_cid"])])
    lf_w_OP = np.array(
        [0.5 * (lf_dict[gid][cid]["OP1"][QoI].dropna().iloc[-1]
                + lf_dict[gid][cid]["OP2"][QoI].dropna().iloc[-1])
         for gid, cid in zip(
            sm_config["optim"]["lf_opt_gid"],
            sm_config["optim"]["lf_opt_cid"])])
    # CoIs
    lf_a_ADP = np.array([lf_dict[gid][cid]["ADP"][CoI].dropna().iloc[-1]
                         for gid, cid in zip(
                             sm_config["optim"]["lf_opt_gid"],
                             sm_config["optim"]["lf_opt_cid"])])
    lf_a_OP1 = np.array([lf_dict[gid][cid]["OP1"][CoI].dropna().iloc[-1]
                         for gid, cid in zip(
                             sm_config["optim"]["lf_opt_gid"],
                             sm_config["optim"]["lf_opt_cid"])])
    lf_a_OP2 = np.array([lf_dict[gid][cid]["OP2"][CoI].dropna().iloc[-1]
                         for gid, cid in zip(
                             sm_config["optim"]["lf_opt_gid"],
                             sm_config["optim"]["lf_opt_cid"])])

    # HF DOE is read and only part of the LF DOE is computed
    mf_sampler = get_sampler(
        n_design, bound, seed, False
    )
    # n_hf + 1 to avoid bug in sampler with zero size
    x_lf_lhs, _ = mf_sampler.sample_mf(n_lf, n_hf + 1)
    ...
    # lf_DOE
    # QoIs
    lf_w_ADP_lhs = np.array([lf_dict[0][cid]["ADP"][QoI].dropna().iloc[-1]
                             for cid in range(len(lf_dict[0]))])
    lf_w_OP_lhs = np.array(
        [0.5 * (lf_dict[0][cid]["OP1"][QoI].dropna().iloc[-1]
                + lf_dict[0][cid]["OP2"][QoI].dropna().iloc[-1])
         for cid in range(len(lf_dict[0]))]
    )
    # CoIs
    lf_a_ADP_lhs = np.array([lf_dict[0][cid]["ADP"][CoI].dropna().iloc[-1]
                             for cid in range(len(lf_dict[0]))])
    lf_a_OP1_lhs = np.array([lf_dict[0][cid]["OP1"][CoI].dropna().iloc[-1]
                             for cid in range(len(lf_dict[0]))])
    lf_a_OP2_lhs = np.array([lf_dict[0][cid]["OP2"][CoI].dropna().iloc[-1]
                             for cid in range(len(lf_dict[0]))])

    # addition of lf baseline results (lf_doe + lf_lhs + baseline)
    # QoI
    x_lf = np.vstack([x_lf, x_lf_lhs, np.zeros(x_lf.shape[-1])])
    lf_w_ADP = np.concatenate(
        [lf_w_ADP, lf_w_ADP_lhs, np.array([sm_config["optim"]["bsl_lf_w_ADP"]])]
    )
    print(f"DEBUG -- lf_w_ADP: {lf_w_ADP}")
    lf_w_OP = np.concatenate(
        [lf_w_OP, lf_w_OP_lhs, np.array([sm_config["optim"]["bsl_lf_w_OP"]])]
    )
    print(f"DEBUG -- lf_w_OP: {lf_w_OP}")
    # CoI
    lf_a_ADP = np.concatenate(
        [lf_a_ADP, lf_a_ADP_lhs, np.array([sm_config["optim"]["bsl_lf_a_ADP"]])]
    )
    lf_a_OP1 = np.concatenate(
        [lf_a_OP1, lf_a_OP1_lhs, np.array([sm_config["optim"]["bsl_lf_a_OP1"]])]
    )
    lf_a_OP2 = np.concatenate(
        [lf_a_OP2, lf_a_OP2_lhs, np.array([sm_config["optim"]["bsl_lf_a_OP2"]])]
    )

    # addition of hf baseline results (hf_doe + baseline)
    # QoI
    x_hf = np.vstack([x_hf, np.zeros(x_hf.shape[-1])])
    hf_w_ADP = np.append(hf_w_ADP, sm_config["optim"]["bsl_hf_w_ADP"])
    print(f"DEBUG -- hf_w_ADP: {hf_w_ADP}")
    hf_w_OP = np.append(hf_w_OP, sm_config["optim"]["bsl_hf_w_OP"])
    print(f"DEBUG -- hf_w_OP: {hf_w_OP}")
    # CoI
