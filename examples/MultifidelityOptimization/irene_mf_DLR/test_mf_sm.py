...
from feasible_acquisition import *
import subprocess
...
def compute_bayesian_infill(
    ...
    **Computes** the low fidelity Bayesian infill candidates.
    ...
    if infill_regularization:
        ...
        infill_lf = maximize_MPI_BO_constrained(model, n_design, bound, seed, infill_nb_gen)
    ...
    infill_lf_LCB_1 = minimize_LCB_constrained(model, 0, n_design, bound, seed, infill_nb_gen)
    infill_lf_LCB_2 = minimize_LCB_constrained(model, 1, n_design, bound, seed, infill_nb_gen)
    ...
    for _ in range(infill_lf_size - 3):
        infill_lf_ED = maximize_ED_constrained(current_DOE, n_design, bound, seed, infill_nb_gen)
...
def main():
    ...
    if pod and ('dlr' in sm_config["study"]['ffd_type']):
        ...
        ffd_pod = DLR_POD_2D(**sm_config["ffd"])
