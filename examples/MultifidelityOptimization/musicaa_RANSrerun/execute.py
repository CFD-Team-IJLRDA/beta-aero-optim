...
def main() -> int:
    ...
    if args_parse.remote:
        ...
        print("Directory: ", os.getcwd().split('/')[-5:])
        cwd = os.getcwd()
        dirName = [s for s in cwd.split('/')[-5:] if 'hf_doe' in s or 'high_infill' in s][0]
        path_to_delete = cwd[:cwd.index(dirName) + len(dirName)]
        path_to_copy = cwd[:cwd.index(dirName)]

        gen = int(re.search(r"high_infill_g(\d+)\.txt", dirName).group(1))
        ...
        stdin, stdout, stderr = ssh.exec_command(f"cd {remote_dir} && rm -rf scripts/ ADP_LES_* OP1_LES_* OP2_LES_* && python3 generate_runs.py -f {dirName}.txt -g {gen} -n 102 && ccc_msub scripts/master_LES.job")
        ...
        if job_id:
            ...
            while True:
                ...
                # check if master_LES.e{job_id} contains "Job terminated"
                if f"master_LES.e{job_id}" in dir_listing:
                    stdin, stdout, stderr = ssh.exec_command(
                        f"grep -c 'Job terminated' {remote_dir}master_LES.e{job_id}"
                    )
                    grep_out = stdout.read().decode().strip()
                    job_terminated = grep_out.isdigit() and int(grep_out) > 0

                if job_id not in job_status and job_terminated == True:
            ...
            if args_parse.copy_irene:
                print(f"\nCopiando i log in un posto sicuro {dirName}...")
                os.system(f'cp {cwd}/musicaa_g0_c0.err {path_to_copy}')
                os.system(f'cp {cwd}/musicaa_g0_c0.out {path_to_copy}')
                print(f"\nScaricando directory {dirName}...")
                os.makedirs(path_to_copy, exist_ok=True)
                scp_download_cmd = f"sshpass -p '{password_irene}' scp -r -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ProxyCommand='ssh -i {key_file_riemann} -W %h:%p {username_riemann}@{hostname_riemann}' {username_irene}@{hostname_irene}:{remote_dir}{dirName} {path_to_copy}"
                os.system(f'rm -r {path_to_delete}')
                os.system(scp_download_cmd)
                print(f"\nDownload command: {scp_download_cmd}")
                print(f"\nDownload completato in {path_to_copy}\n")
