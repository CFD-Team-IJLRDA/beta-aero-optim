...
FAILURE: int = 1
SUCCESS: int = 0
MUSICAA: str = "mpiexec -n 45 /home/mciarlatani/bin/musicaa"
...
def execute_unsteady(config: dict, sim_outdir: str):
    ...
    **Executes** a Large Eddy Simulation with MUSICAA.
    """
    if not os.path.isdir(os.path.join(sim_outdir, "init_2D")):
        ...
        print(f"{os.path.join(sim_outdir, 'init_2D')} found, 2D phase skipped")

    # carry on with 3D transient
    if not os.path.isdir(os.path.join(sim_outdir, "init_3D")):
        ...
        print(f"{os.path.join(sim_outdir, 'init_3D')} found, 3D transient phase skipped")

    # gather statistics
    if not os.path.isfile(os.path.join(sim_outdir, "QoI_convergence.csv")):
        config.update({"is_stats": True})
        ...
        print(f"{os.path.join(sim_outdir, 'QoI_convergence.csv')} found, 3D stat. phase skipped")
...
def monitor_sim_progress(proc: subprocess.Popen,
    ...
    **Monitors** a simulation.
    ...
    divide_CFL_by = config["simulator"].get("divide_CFL_by", 1.4)
    ...
    while True:
        ...
        # computation has crashed
            ...
            if current_restart < restart:
                ...
                print(f"ERROR -- {unsteady_step} crashed with CFL={CFL} "
...
def main() -> int:
    """
    This program runs a MUSICAA CFD simulation at ADP.
    ...
    parser.add_argument("-remote", action="store_true", help="remote simulation")
    ...
    if args_parse.remote:

        # opt_dir = os.path.join(os.getcwd(), "..", "..")
        opt_dir = '/'.join(os.getcwd().split('/')[:-2])
        opt_mother_dir = '/'.join(os.getcwd().split('/')[:-3])
        print('Home directory: ', opt_dir)
        print('Pre-home directory: ', opt_mother_dir)

        print("Directory: ", os.getcwd().split('/')[-3:])
        idx_dirName = os.getcwd().split('/')[-3:].index('MUSICAA')
        cwd = os.getcwd()

        dirName = os.getcwd().split('/')[-3:][idx_dirName-1]
        print("dirName: ", dirName)
        path_to_delete = cwd[:cwd.index(dirName) + len(dirName)]
        path_to_copy = cwd[:cwd.index(dirName)]

        matched_files = glob.glob(os.path.join(opt_dir, f"{dirName}_g*.txt"))
        if not matched_files:
            raise FileNotFoundError(f"No file matching {dirName}_g*.txt found in {opt_dir}")
        gen = max(
            int(re.search(rf"{re.escape(dirName)}_g(\d+)\.txt", f).group(1))
            for f in matched_files
        )
        print('====================== Running remotely Generation ', gen)

        fileName = os.path.join(path_to_delete, dirName)

        print("\nPath to delete: ", path_to_delete)
        print("Path to copy: ", path_to_copy)
        print("Filename to copy: ", fileName+'.txt')
        print('====================== File to copy is ', fileName+'.txt')
        ...
        scp_cmd = f"sshpass -p '{password_irene}' scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ProxyCommand='ssh -i {key_file_riemann} -W %h:%p {username_riemann}@{hostname_riemann}' {fileName}_g{gen}.txt {username_irene}@{hostname_irene}:{remote_dir}"
        os.system(scp_cmd)
        print("Comand to copy: ", scp_cmd, "\n\n")
        time.sleep(5)
        ...
        print("\nEseguendo setup, generazione runs, e lancio la simulazione...\n")
        stdin, stdout, stderr = ssh.exec_command(f"cd {remote_dir} && rm -rf scripts/ && python3 generate_runs.py -f {dirName}_g{gen}.txt -g {gen} -n 102 && ccc_msub scripts/master_LES.job")
        ...
        while True:
            ...
            # check if master_LES.e{job_id} in opt_mother_dir contains "Job terminated"
            error_file = os.path.join(opt_mother_dir, f"master_LES.e{job_id}")
            if os.path.isfile(error_file):
                with open(error_file, "r") as ef:
                    job_terminated = "Job terminated" in ef.read()

            if job_terminated==True:
                print("Job terminato e voila!")
                break

            time.sleep(30)

        # print("======================= Copy instructions ======================= ")
        # print(f'rm -r {opt_dir}/MESH/*_g{gen}_* && cp -r {opt_dir}_g{gen}/MESH/* {opt_dir}/MESH')
        # print(f'rm -r {opt_dir}/FFD/*_g{gen}_* && cp -r {opt_dir}_g{gen}/FFD/* {opt_dir}/FFD')
        # print(f'rm -r {opt_dir}/MUSICAA/*_g{gen}_* && cp -r {opt_dir}_g{gen}/MUSICAA/musicaa_g{gen}* {opt_dir}/MUSICAA/')
        # time.sleep(1000)

        os.system(f'rm -r {opt_dir}/MESH/*_g{gen}_* && cp -r {opt_dir}_g{gen}/MESH/* {opt_dir}/MESH')
        os.system(f'rm -r {opt_dir}/FFD/*_g{gen}_* && cp -r {opt_dir}_g{gen}/FFD/* {opt_dir}/FFD')
        # os.system(f'rm -r {opt_dir}/MUSICAA/*_g{gen}_*')
        ...
        # ssh.exec_command( f"cp -r {remote_dir}/{dirName}_g{gen}/MUSICAA/musicaa_g{gen}*  {remote_dir}/{dirName}/MUSICAA")
        # ssh.close()

        # os.system(f'cp -r {opt_dir}_g{gen}/MUSICAA/musicaa_g{gen}* {opt_dir}/MUSICAA/')
        # time.sleep(1000)

        # os.system(f'rsync -av --size-only {opt_dir}_g{gen}/MESH/ {opt_dir}/MESH')
        # os.system(f'rsync -av --size-only {opt_dir}_g{gen}/FFD/ {opt_dir}/FFD')
        # os.system(f'rsync -av --size-only {opt_dir}_g{gen}/MUSICAA/ {opt_dir}/MUSICAA/')
