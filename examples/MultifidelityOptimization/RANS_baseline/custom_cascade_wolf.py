...
class CustomMesh(CascadeMesh):
    ...
    def reorder_blade(self) -> list[list[float]]:
        ...
        **Returns** the blade profile after reordering in clockwise direction.
        ...
        if y1 > y0:
            ...
            return [[p[0], p[1], p[2]] for p in (self.pts[start:] + self.pts[:start])[::-1]]
    ...
    def build_2dmesh(self):
        ...
        **Builds** the surface mesh of the computational domain.
        ...
        # spl_le = gmsh.model.geo.addSpline(pt_wall[272 - 1:len(self.pts)] + [pt_wall[0]] + pt_wall[:5])
        ...
        # blade splines and transfinite curves
        # 35 -> 5, 287 -> 272 and 322 -> 275
        spl_le = gmsh.model.geo.addSpline(pt_wall[269 - 1:len(self.pts)] + [pt_wall[0]] + pt_wall[1:10])
        # 88 -> 69, 129 -> 117 and 157 -> 147
        spl_ss = gmsh.model.geo.addSpline(
            pt_wall[10 - 1:69] + pt_wall[69 - 1:117] + pt_wall[117 - 1:143]
        )
        # 157 -> 147, 168 -> 153 and 179 -> 163
        spl_te = gmsh.model.geo.addSpline(pt_wall[143 - 1:153] + pt_wall[153 - 1:167])
        # 179 -> 163, 245 -> 232 and 287 -> 272
        spl_ps = gmsh.model.geo.addSpline(pt_wall[167 - 1:232] + pt_wall[232 - 1:269])
        gmsh.model.geo.mesh.setTransfiniteCurve(spl_le, self.le, "Progression", 1.0)
        gmsh.model.geo.mesh.setTransfiniteCurve(spl_ss, self.nodes_ss, "Progression", 1.0)
        gmsh.model.geo.mesh.setTransfiniteCurve(spl_te, self.te, "Progression", 1)
        gmsh.model.geo.mesh.setTransfiniteCurve(spl_ps, self.nodes_ps, "Progression", 1.0)
        ...
        # [n1, n2, n3, n4, n5, n6, n7, n8] = [10,69,117,143,167,230,268,len(self.pts)]
        # spl_1 = gmsh.model.geo.addSpline(pt_wall[n7 - 1:n8] + pt_wall[:n1]) # Leading edge
        # spl_2 = gmsh.model.geo.addSpline(pt_wall[n1 - 1:n2]) # 1st suction
        # spl_3 = gmsh.model.geo.addSpline(pt_wall[n2 - 1:n3]) # 2nd suction
        # spl_4 = gmsh.model.geo.addSpline(pt_wall[n3 - 1:n4]) # 3rd suction
        # spl_5 = gmsh.model.geo.addSpline(pt_wall[n4 - 1:n5]) # Trailing edge
        # spl_6 = gmsh.model.geo.addSpline(pt_wall[n5 - 1:n6]) # 1st pressure
        # spl_7 = gmsh.model.geo.addSpline(pt_wall[n6 - 1:n7]) # 2nd pressure
