...
def main():
    ...
    parser.add_argument("-i", "--input", type=str, help="input results directory")
    parser.add_argument(
        "-mp", "--mplan", action='store_true', help="extract results along MP1 and MP2"
    )
    parser.add_argument("-o", "--output", type=str, help="output results directory")
    ...
    # save data
    pvs.SaveData(
        os.path.join(args.output, f"plot1.csv"),
        proxy=plotOverLine1,
    ...
    # save data
    pvs.SaveData(
        os.path.join(args.output, f"plot2.csv"),
        proxy=plotOverLine2,
        PointDataArrays=[' Scalar1', ' Scalar2', ' Vector2', 'Reference', 'arc_length', 'vtkValidPointMask'] # noqa
    )
