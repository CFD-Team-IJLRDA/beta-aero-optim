...
def plot_opt_comparison(mf_df, hf_df, bsl_w_ADP, bsl_w_OP, selected=None):
    ...
    sc1 = ax.scatter(mf_df.loc[mf_df["infill"] != 0, ["w_ADP"]], mf_df.loc[mf_df["infill"] != 0, ["w_OP"]], marker='o', cmap=cmap_mf, c=colors_mf[n_DOE:], s=size, label="MF infill")
    sc2 = ax.scatter(hf_df.loc[hf_df["infill"] != 0, ["w_ADP"]], hf_df.loc[hf_df["infill"] != 0, ["w_OP"]], marker='v', cmap=cmap_hf, c=colors_hf[n_DOE:], s=size, label="HF infill")
    ax.scatter(mf_df.loc[mf_df["infill"] == 0, ["w_ADP"]], mf_df.loc[mf_df["infill"] == 0, ["w_OP"]], marker='s', color='k', s=size, label="initial DOE")
    
    ax.scatter(bsl_w_ADP, bsl_w_OP, marker="+", color="k", s=size, label="HF baseline")
    
    ax.scatter(mf_unfeas.loc[mf_unfeas["infill"] != 0, ["w_ADP"]], mf_unfeas.loc[mf_unfeas["infill"] != 0, ["w_OP"]], marker='o', c='red', s=size, label='Unfeasible point')
    ...
    cbar1 = plt.colorbar(sc1, ax=ax, ticks=[1, 5, 10, 15], pad=0.005, fraction=0.035)
    cbar1.set_label('Infill number')
    
    cbar2 = plt.colorbar(sc2, ax=ax, ticks=[], pad=0.02, fraction=0.040)
    # # legend
    ax.legend(loc="lower right")
...
def read_lf_results(dir, n_pop, n_gen, check=False):
    ...
    for gen in range(n_gen):
        for ind in range(n_pop):
            ...
            df_temp.loc[len(df_temp)] = [gen, ind+1, loss_ADP, (loss_OP1+loss_OP2)*0.5, loss_OP1, loss_OP2, beta_ADP, beta_OP1, beta_OP2,
...
def plot_opt_mf(lf_df, hf_df, nLFtoHF, bsl_w_ADP, bsl_w_OP, selected=None):
    ...
    for point in selected:
        ...
        ax.scatter(x_HF, y_HF, marker="o", facecolors='none', edgecolors='red', linewidth=1.5, s=size*1.5)
        ax.scatter(x_LF, y_LF, marker="X", facecolors='none', edgecolors='red', linewidth=1.5, s=size*1.5)
        
        txt = plt.text(x_HF-0.002, y_HF-0.0055, str(idx+1), fontsize=6, fontweight='bold',
                    color='r', ha='left', va='bottom')
        idx+=1

        ax.plot(x_line, y_line, color='tab:grey', linestyle='dashed', linewidth=1)
    
    ax.scatter(hf_df.loc[hf_df["infill"] == 0, ["w_ADP"]], hf_df.loc[hf_df["infill"] == 0, ["w_OP"]], marker='s', color='k', s=size, label="Initial HF DOE")
    
    ax.scatter(hf_df.loc[hf_df["infill"] != 0, ["w_ADP"]][::-1], hf_df.loc[hf_df["infill"] != 0, ["w_OP"]][::-1], marker='o', cmap=cmap_hf, c=hf_df.loc[hf_df["infill"] != 0, ["infill"]].to_numpy()[::-1], s=size, label="HF infill", alpha=0.8)
    sc = ax.scatter(lf_df[["w_ADP"]], lf_df[["w_OP"]], marker='X', cmap=cmap_lf, c=lf_df[["infill"]].to_numpy(), s=size, label="LF infill")
    ax.scatter(hf_unfeas.loc[hf_unfeas["infill"] != 0, ["w_ADP"]], hf_unfeas.loc[hf_unfeas["infill"] != 0, ["w_OP"]], marker='o', c='red', s=size, label='Unfeasible point')
    ...
    axins.scatter(hf_df.loc[hf_df["infill"] != 0, ["w_ADP"]][::-1], hf_df.loc[hf_df["infill"] != 0, ["w_OP"]][::-1], marker='o', cmap=cmap_hf, c=hf_df.loc[hf_df["infill"] != 0, ["infill"]].to_numpy()[::-1], s=size, alpha=0.8)
    axins.scatter(lf_df[["w_ADP"]], lf_df[["w_OP"]], marker='X', cmap=cmap_lf, c=lf_df[["infill"]].to_numpy(), s=size)
    axins.scatter(hf_unfeas.loc[hf_unfeas["infill"] != 0, ["w_ADP"]], hf_unfeas.loc[hf_unfeas["infill"] != 0, ["w_OP"]], marker='o', c='red', s=size)
    axins.scatter(hf_unfeas.loc[hf_unfeas["infill"] == 0, ["w_ADP"]], hf_unfeas.loc[hf_unfeas["infill"] == 0, ["w_OP"]], marker='s', c='red', s=size)
    axins.scatter(bsl_w_ADP, bsl_w_OP, marker="+", color="k", s=size)
    ...
    for point in selected:
        ...
        axins.scatter(x_HF, y_HF, marker="o", facecolors='none', edgecolors='red', linewidth=1.0, s=size*1.5)
        axins.text(x_HF-0.0010, y_HF-0.0015, str(idx+1), fontsize=6, fontweight='bold', color='r', ha='left', va='bottom')
        idx+=1
    
    # Imposta i limiti dello zoom
    axins.set_xlim(0.025, 0.036)
    axins.set_ylim(0.037, 0.059)
    ...
    cbar = plt.colorbar(sc, ax=ax, ticks=[1, 5, 10, 15], pad=0.02, fraction=0.050)
    cbar.set_label('Infill number')
...
def plot_scatter_modes(df_opt, baseline, figsize=[10, 5], fontsize=16, chosen=None):
    ...
    xlim={'x5':[-0.05,0.05], 'x4':[-0.1,0.1], 'x3':[-0.1,0.1], 'x2':[-0.075,0.075], 'x1':[-0.1,0.1]}
    # xlim={'x5':[-0.1,0.1], 'x4':[-0.1,0.1], 'x3':[-0.1,0.1], 'x2':[-0.1,0.1], 'x1':[-0.1,0.1]}
    ylim={'w_ADP':[0.022,0.062], 'w_OP1':[0.03,0.13], 'w_OP2':[0.022,0.062]}
    ...
    for i in range(0,15):
        ...
        if i < 5:
            argy = 'w_ADP'
            plt.plot(xlim[argx], [bsl_w_ADP, bsl_w_ADP], 'k--', linewidth=0.5)
        elif i < 10:
            argy = 'w_OP1'
        ...
        plt.scatter(df_opt.loc[df_opt["infill"] == 0, [argx]], df_opt.loc[df_opt["infill"] == 0, [argy]], marker="s", s=size, c=df_opt.loc[df_opt["infill"] == 0, ["infill"]].to_numpy())
        plt.scatter(df_opt.loc[df_opt["infill"] != 0, [argx]], df_opt.loc[df_opt["infill"] != 0, [argy]], marker="o", s=size, c=df_opt.loc[df_opt["infill"] != 0, ["infill"]].to_numpy())
        ...
        if not(chosen is None):
            for point in chosen:
                ...
                txt = plt.text(x_HF+dx, y_HF+dy, str(idx+1), fontsize=6, fontweight='bold',
                            color='r', ha='left', va='bottom')
                idx+=1

        if i == 0:
            plt.ylabel(r'$\mathcal{L}_{ADP}$')
        elif i == 5:
            plt.ylabel(r'$\mathcal{L}_{OP1}$')
        elif i == 10:
            plt.ylabel(r'$\mathcal{L}_{OP2}$')
        else:
            plt.yticks([])

        if i in [10,11,12,13,14]:
            plt.xticks([np.min(xlim[argx]), 0, np.max(xlim[argx])])
            plt.xlabel(r'$x_'+str(i-9)+'$')
        else:   
            plt.xticks([])
...
def plot_chosen_profiles(mf_df, baseline, selected, figsize=[8, 2.5]):
    ...
    for point in selected:
        ...
        ax.plot(pro[:, 0], pro[:, 1], linewidth=1, label=lab)
    ...
    axins1.set_xticks([])
    axins1.set_yticks([])
    axins2.set_xlim(max(baseline[:, 0]) - 1 * Delta, max(baseline[:, 0]) + 0.1 * Delta)
...
def main():
    ...
    plot_chosen_profiles(mf_hf_opt, baseline, selected)

    # # lf_opt = read_lf_results(lf_dir, 30, 50)
    # # lf_opt = lf_opt.loc[lf_opt['unfeas_geom']==False].reset_index(drop=True)
    # # lf_fit = np.vstack([[lf_opt['w_ADP'].astype(float).to_numpy().flatten()], [lf_opt['w_OP'].astype(float).to_numpy().flatten()]]).T
    # # lf_pareto = compute_pareto(lf_fit[:, 0], lf_fit[:, 1])
    # # lf_pareto_idx = [np.where(lf_fit == pid)[0][0] for pid in lf_pareto]  
    # # df_pareto = lf_opt.iloc[lf_pareto_idx].reset_index(drop=True)
    ...
    # # plot_scatter_modes(mf_lf_opt, [lf_bsl_w_ADP, lf_bsl_w_OP1, lf_bsl_w_OP2])
