...
def plot_grid(
        ...
        n_bl: int = 0, every: int = 5,
        ngh: int = 5, figsize: tuple[float, float] = (5.2, 3.64),
        xlabel: str = "$x$ [mm]", ylabel: str = "$y$ [mm]"
):
    # General information
    dict_info = read_info(dir_data)
    n_bl = dict_info['nbloc'] if not n_bl else n_bl
    ...
    for bl_num in range(n_bl):
        ...
        if is_curv:
            ...
            plt.vlines(x[0], ymin=y[0, 0], ymax=y[-1, 0], colors='black', linewidth=1)
            plt.hlines(y[:, 0], xmin=x[0, 0], xmax=x[0, -1], colors='black', linewidth=1)
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)
    plt.show()
